...
|---|---|
| `docx/` | The three Word originals. `Judgment_Layer_Jev_Usage_Catalog.docx` is the final version (footnotes, figure). `jev_usages.agent.final.footnote.docx` is the same text one build earlier. `jev_usages.agent.final.base.docx` has inline citation marks instead of footnotes. |
